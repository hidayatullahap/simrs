                ...
                <div class="box">
                    <div class="box-header with-border">
                        ...
                        <table class="table table-bordered table-hover" id="tabel" cellspacing="0" width="100%">
                            ...
                            <tr>
                                <th><font color="white">Tanggal Transaksi</th>
                                <th><font color="white">Nama</th>
                                <th><font color="white">Nomor Transaksi</th>
                                <th><font color="white">Total Tagihan</th>
                                <th><font color="white">Detil</th>
                            </tr>
                            </thead>
                            <tbody>
                            <?php
                            $url=base_url('/depo/resepkeluar/detiltransaksi');
                            if ($data->num_rows>0) {
                                foreach ($data as $field => $values) {
                                    echo "<tr>";
                                    $date=strtotime($values['tanggal_transaksi']);
                                    echo "<td width='10%'>".date('d M Y H:i:s', $date)."</td>";
                                    echo "<td>".$values['nama']."</td>";
                                    echo "<td>".$values['nomor_transaksi']."</td>";
                                    echo "<td>".$values['total_tagihan']."</td>";
                                    $nomorTransaksi = $values['nomor_transaksi'];
                                    echo "<td width='4%'><button type='button' class='btn btn-primary btn-sm' 
                                    onclick='window.location.href=\"$url/$nomorTransaksi\"'>Detil</button></td>";
                                    echo "</tr>";
                                }
                            } else {
                    ...
                    <span><i>*Hapus 'filter mulai tanggal' untuk mendapatkan semua hasil</i></span>
                        ...
                        if (isset($totalPages)) {
                            $entity->tampilkan('depo/resepkeluar',$currentPage, $totalPages);
                        }
                        ?>
                    </div>
