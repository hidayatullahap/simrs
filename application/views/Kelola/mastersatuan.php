...
<div class="content-wrapper">
    <section class="content-header">
        <h1>
            <a href="<?php echo base_url('/kelola/mastersatuan'); ?>"><font color='black'><strong>Kelola - Satuan</strong></font></a>
        </h1>
    </section>
                ...
                <div class="box">
                    <div class="box-header with-border">
                        <div class="form-group row">
                            <div class="col-xs-2">
                                <button type="button" class="btn btn-info btn-md" id="buttonTambah" data-toggle="modal" data-target="#addForm">Tambah Satuan</button>
                            </div>
                            <form method="post" action="<?php echo base_url('/kelola/mastersatuan/search') ?>">
                            <div class="input-group col-xs-2" style="float: right;padding-right:15px;">
                            <input  type="text" class="form-control" autocomplete="off" placeholder="Cari nama Satuan" name="search" id="search">
                        ...
                        <table class="table table-bordered table-hover" id="tabel" cellspacing="0" width="100%">
                            ...
                            <tr>
                                <th><font color="white">Satuan ID</th>
                                <th><font color="white">Nama Satuan</th>
                            ...
                            if ($data->num_rows>0) {
                                ...
                                foreach ($data as $field => $values) {
                                    echo "<tr>";
                                    echo "<td width='8%'>".$values['satuan_id']."</td>";
                                    echo "<td>".$values['nama_satuan']."</td>";

                                    echo "<td><a data-toggle='tooltip' onclick='editModal($i);' title='edit'><i class='fa fa-fw fa-edit'></i></a>
                                    <a data-toggle='tooltip' title='hapus'><i class='fa fa-fw fa-remove' data-toggle='modal' data-target='.bs-example-modal-sm' data-id='".$values['satuan_id']."' 
                                    data-nama='".$values['nama_satuan']."'></i></a></td>";
                                    echo "</tr>";
                                    $i++;
                                }
                            } else {
                    ...
                    <form method="post" id="formModal" action="<?php echo base_url('/kelola/mastersatuan/insertdata') ?>">
                    <div class="modal fade" id="addForm" role="dialog">
                        <div class="modal-dialog modal-md">
                            <div class="modal-content">
                                <div class="modal-header">
                                <button type="button" class="close" data-dismiss="modal">&times;</button>
                                <h4 class="modal-title" id="headerModal">Tambah Satuan</h4>
                                </div>

                                <div class="modal-body" style="text-align: right; ">
                                    <div class="item form-group">
                                        <label class="col-md-4 control-label paddingForm" for="nama_satuan">Nama Satuan</label>
                                        <div class="col-md-6">
                                            <?php
                                            $data = array(
                                                'name' => 'nama_satuan',
                    ...
                    <div id="delete" class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                        <div class="modal-dialog modal-sm">
                            ...
                            <div class="modal-body">
                                Anda yakin ingin menghapus data Satuan <br>
                                Nama : <strong class="modal-Nama">Nama Satuan</strong>
                            </div>
                            <div class="modal-footer">
                    ...
                    <div class="box-footer clearfix">
                        ...
                        if (isset($totalPages)) {
                            $entity->tampilkan('kelola/mastersatuan',$currentPage, $totalPages);
                        }
                        ?>
                    </div>
                    <?php if($this->session->flashdata('pesan')) {?>
                        <div class="alert alert-success alert-dismissible" id="success-alert">
                            ...
                            <h4><i class="icon fa fa-info"></i> Data Satuan</h4>
                            <?php echo $this->session->flashdata('metode')." data Satuan ".$this->session->flashdata('pesan'); ?>
                        </div>
                    <?php } ?>
                    </div>
                </div>
...
<script>
    $('#delete').on('show.bs.modal', function (event) {
        ...
        document.getElementById("deletelink").href="<?php echo base_url('kelola/mastersatuan/deletedata/'); ?>"+"/"+dataID;
    });
</script>
...
$(document).ready(function(){
    $("#buttonTambah").click(function(){
        document.getElementById("headerModal").innerHTML = "Tambah Satuan";
        document.getElementById("submitModal").innerHTML = "Simpan";
        document.getElementById("formModal").action ="<?php echo base_url('/kelola/mastersatuan/insertdata') ?>";
    });
});
...
function editModal(row) {
    ...
    document.getElementById("nama_satuan").value = document.getElementById("tabel").rows[row].cells[1].innerHTML;

    document.getElementById("headerModal").innerHTML = "Edit Satuan";
    document.getElementById("submitModal").innerHTML = "Simpan Perubahan";
    document.getElementById("formModal").action ="<?php echo base_url('/kelola/mastersatuan/editdata') ?>"+"/"+id;
    $("#addForm").modal();
}
</script>
